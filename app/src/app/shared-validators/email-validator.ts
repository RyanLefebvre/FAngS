...
import { hasWhiteSpace } from './validator-util-functions/has-white-space';
...
export function ValidateEmail(control: AbstractControl): {} {
  ...
  if (emailIsNull) {
    ...
    if (!emailIsString) {
      ...
      const emailIsValidLength: boolean =
        MIN_LENGTH <= emailLength && emailLength <= MAX_LENGTH;
      const emailContainsAt: boolean = email.includes('@');
      const emailHasNoWhiteSpace = !hasWhiteSpace(email);
      const emailIsValid: boolean =
        emailIsValidLength && emailContainsAt && emailHasNoWhiteSpace;
